...
export default function SessionLog({ sessions }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {expanded ? (
        <SessionDetail session={expanded} onBack={() => setExpandedId(null)} />
      ) : (
...
function SessionCard({ session, number, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left bg-council-card border border-council-border rounded-xl px-5 py-4 hover:border-council-border-light transition-colors group"
    >
      ...
      <div className="flex items-center gap-3 mt-3 text-xs text-council-text-dim">
        ...
        <span className="ml-auto text-council-text-dim/40 group-hover:text-council-accent/60 transition-colors text-[11px]">
          View →
        </span>
      </div>
    </button>
...
function SessionDetail({ session, onBack }) {
  ...
  return (
    <div>
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-xs text-council-text-dim hover:text-council-text transition-colors mb-5"
      >
